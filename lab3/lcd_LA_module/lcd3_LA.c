...
static int 
open_lcd3_LA(struct inode *inode, struct file *file) 
{
    printk(KERN_DEBUG "LCD3 initialised!\n");               //Print a debug message indicating that the LCD3 has been initialized
    printk(KERN_DEBUG "LCD3 can be write on!\n");           //Print a debug message indicating that the LCD3 is ready to be written on
    return 0;                                               //Return 0 to indicate successful completion
}

//This function is called when the device file is read from with read system call
static ssize_t 
read_lcd3_LA(struct file *file, char *buf, size_t count, loff_t *ppos) 
{
    printk(KERN_DEBUG "ERROR: Can not read on lcd3!\n");    //Print a debug message indicating that reading from the LCD3 is not supported
...
static ssize_t 
write_lcd3_LA(struct file *file, const char *buf, size_t count, loff_t *ppos) {
    
    printk(KERN_DEBUG "Writing on lcd3!\n");                //Print a debug message indicating the value that is being written to the LCD3
...
static int 
release_lcd3_LA(struct inode *inode, struct file *file) 
{
    printk(KERN_DEBUG "LCD file closed!\n");            //Print a debug message indicating that the LCD3 has stopped working
    return 0;                                           //Return 0 to indicate successful completion
}

// Definition of file operations for LCD3 character device driver
struct file_operations fops_lcd3_LA = {
    .open       = open_lcd3_LA,                         // function to be called when the device is opened
    .read       = read_lcd3_LA,                         // function to be called when the device is read
    .write      = write_lcd3_LA,                        // function to be called when the device is written to
    .release    = release_lcd3_LA                       // function to be called when the device is closed
};
...
static int __init mon_module_init(void)
{
    ...
    major = register_chrdev(0, "lcd3_LA", &fops_lcd3_LA);   // 0 To let linux choose the major number
    printk(KERN_DEBUG "lcd3_LA connected!\n");              // Indicates the start of the module
...
static void __exit mon_module_cleanup(void)
{
    ...
    unregister_chrdev(major, "lcd3_LA");                    // Function to unload the driver
    printk(KERN_DEBUG "lcd3_LA deconnected!\n");            // Indicates the end of the module
}
