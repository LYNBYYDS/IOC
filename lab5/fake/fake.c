...
int main()
{
    ...
    char    *f2sName = "/tmp/f2s_LA";                       // filo names
    char    *s2fName = "/tmp/s2f_LA";                       //
    ...
    do {
        ...
        FD_SET(0, &rfds);                                   // On change le masque pour que le descripteur de fichier stdin soit surveiller par select
        ...
        if (select(s2f+1, &rfds, NULL, NULL, &tv) != 0) {   // wait until timeout
            ...
            if (FD_ISSET(0, &rfds)) {                     // on verifie si stdin est disponnible pour la lecture
                int nbchar;
