...
open_led0_LA(struct inode *inode, struct file *file) 
{
    ...
    gpio_fsel(GPIO_LED0, GPIO_FSEL_OUTPUT);                 // Setup the GPIO pin for LED0 as an output pin
...
{    
    printk(KERN_DEBUG "Writing on led0! Value input : %d\n", buf[0]);       // Print a debug message indicating the value that is being written to the LED0
    ...
    if (buf[0] == '0') {
        ...
        gpio_write(GPIO_LED0, 0);                   // Turn off the LED0 by writing 0 to the GPIO pin
    }   
    // If the value being written is '1', turn on the LED0
        ...
        gpio_write(GPIO_LED0, 1);                   // Turn on the LED0 by writing 1 to the GPIO pin
    }
